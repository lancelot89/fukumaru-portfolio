    ...
    <div className="space-y-4">
      <div>
        ...
        <input id="q" value={q} onChange={(e) => setQ(e.target.value)}
          className="w-full rounded border px-3 py-2 bg-white dark:bg-slate-900" placeholder="例: GCP, Go" />
      </div>
      <ul className="grid md:grid-cols-2 gap-4">
        {filtered.map((p) => (
          <li key={p.slug} className="rounded border p-4">
            <a href={`/blog/${p.slug}`} className="block focus:outline-none focus:ring-2 focus:ring-brand/60 rounded">
